...
```{r}
cic_list_location_only <- merged_cleaned %>% 
  filter(form != "GOV" & level == "provider") %>% 
  select(provider_name, location_name, level, form_spinout, service_type) 
```

```{r}
cic_list_provider_2level_repeat <- merged_cleaned %>% 
  filter(form != "GOV") %>% 
  select(provider_name, level, form_spinout, service_type) %>% 
  group_by(provider_name, level) %>% 
  slice_head(n = 1)
```


```{r}
write_csv(cic_list_location_only, file = "cic_list_location_only.csv")
```

```{r}
write_csv(cic_list_provider_2level_repeat, file = "cic_list_provider_2level_repeat.csv")
```
