...
subtitle: "version after July 2024 discussions"
author: "Meng Ye"
date: "7/20/2024"
output:
...
## This version mainly inclue the models we decided with after the July 2024 meetings 
1. Include fixed effects for service types 
2. year either as numeric or as factors
...
```{r}
write_csv(merged_cleaned, here("cleaned_data", "spinout_2017.csv"))
```
...
count_by_level <- merged_cleaned %>% 
  ...
  summarize(count_provider = n_distinct(provider_name),
            count_location = n_distinct(location_name) - 1, #subtract the provider NA count
            count_overall_rating = sum(overall),
            count_rating = n()) 

sum_by_level <-count_by_level %>%   
  summarise(across(where(is.numeric), sum)) %>%
  mutate(form_spinout = "Total")

count_by_form <- bind_rows(count_by_level, sum_by_level)
count_by_form
...
```{r}
count_by_level1 <- merged_cleaned %>% 
    filter(level == "provider") %>%
  mutate(overall = ifelse(domain == "Overall", 1, 0)) %>% 
  group_by(form_spinout) %>% 
  summarize(count_provider = n_distinct(provider_name),
...
sum_by_level1 <-count_by_level1 %>%   
  summarise(across(where(is.numeric), sum)) %>%
  mutate(form_spinout = "Total")

count_by_form1 <- bind_rows(count_by_level1, sum_by_level1)
count_by_form1
```


```{r}
count_by_level2 <- merged_cleaned %>% 
...
sum_by_level2 <-count_by_level2 %>%   
  summarise(across(where(is.numeric), sum)) %>%
  mutate(form_spinout = "Total")

count_by_form2 <- bind_rows(count_by_level2, sum_by_level2)
count_by_form2
```

```{r}
table(merged_cleaned$level)
```
```{r}
sum(is.na(merged_cleaned$service_group))
```
...
```{r}
count_provider_data <- merged_cleaned %>% 
  filter(level == "provider") %>%
  mutate(overall = ifelse(domain == "Overall", 1, 0)) %>% 
  group_by(form_spinout) %>% 
  summarize(count_provider = n_distinct(provider_name),
...
```{r}
# count the rating units by level
 rating_unit %>% 
  group_by(level) %>% 
  summarise(rating_unit = n()) 
```
...
### 4.1 whole models with continous year trend  - service tyep fixed effects,  **THE MODEL WE DECIDE TO USE!**
...
### 4.1 whole models with Year fixed  - service tyep fixed effects

Since service_type is inclusive of variation/information from `social_care` and `level`, we drop the variables from the model 


```{r }
model_order_overall2 <- clm(rating ~ form_spinout  + service_type + region + Year,
                data = filter(merged_cleaned, domain == "Overall"),
                link = "logit")

model_order_safe2 <- clm(rating ~ form_spinout  + service_type + region + Year,
                data = filter(merged_cleaned, domain == "Safe"),
                link = "logit")
model_order_effective2 <- clm(rating ~ form_spinout  + service_type + region + Year,
                data = filter(merged_cleaned, domain == "Effective"),
                link = "logit")
model_order_caring2 <- clm(rating ~ form_spinout  + service_type + region + Year,
                data = filter(merged_cleaned, domain == "Caring"),
                link = "logit")
model_order_well_led2 <- clm(rating ~ form_spinout + service_type + region + Year,
                data = filter(merged_cleaned, domain == "Well-led"),
                link = "logit")
model_order_responsive2 <- clm(rating ~ form_spinout + service_type + region + Year,
                data = filter(merged_cleaned, domain == "Responsive"),
                link = "logit")
```
